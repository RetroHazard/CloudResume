...
export function StatusPill({
    ...
    return (
        <span
            title={title}
            className={cn(
                'inline-flex items-center gap-1.5 font-mono text-[0.62rem] font-semibold tracking-[0.14em] uppercase',
                text,
            )}
        >
            ...
            {(kicker || title) && (
                ...
                <header className='flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5 border-b border-border bg-secondary-800/60 px-5 py-3 sm:px-7'>
                    <div className='flex min-w-0 items-center gap-3'>
                        {(badge || title) && <LineBadge letter={badge ?? title!.charAt(0)} color={c} />}
                        <div className='flex min-w-0 flex-col'>
                            {kicker && (
                                <span className='font-mono text-[0.6rem] font-semibold tracking-[0.24em] text-content-accent uppercase'>
                                    {kicker}
                                </span>
                            )}
                            {title && (
                                <TitleTag className='mb-0 font-heading text-xl leading-none font-extrabold tracking-wide break-words text-content-header uppercase sm:text-3xl'>
                                    {title}
                                </TitleTag>
                            )}
                        </div>
...
export function Board({ columns, children }: { columns: string[]; children: ReactNode }) {
    const left = columns.slice(0, -1).join('  ·  ');
    const right = columns[columns.length - 1];
    return (
        <div className='overflow-hidden rounded-lg border border-border'>
            <div className='flex items-center justify-between gap-3 border-b border-border bg-secondary-800/70 px-3 py-2 font-mono text-[0.58rem] font-semibold tracking-[0.16em] text-content-accent uppercase sm:px-4'>
                <span>{left}</span>
...
export function DepartureRow({
    ...
    return (
        ...
        >
            <summary className='grid cursor-pointer list-none [grid-template-columns:auto_1fr_auto] items-center gap-3 px-3 py-3 transition-colors hover:bg-secondary-800/50 sm:px-4'>
                ...
                <span className='flex items-center gap-3'>
                    <span className='flex flex-col items-end max-sm:hidden'>
                        <span className='tabular font-mono text-xs font-semibold text-content-subtitle'>{since}</span>
                        <StatusPill tone={status}>
                            {statusLabel ?? (status === 'on' ? 'On Time' : 'Departed')}
                        </StatusPill>
                    </span>
            ...
            <div className='border-t border-border/60 bg-background/40 px-3 pt-3 pb-4 sm:px-4'>{children}</div>
        </details>
    );
}
...
export function Chip({ children, className }: { children: ReactNode; className?: string }) {
    return (
        <span
            className={cn(
                'inline-flex items-center gap-1.5 rounded border border-border bg-secondary-800/70 px-2 py-0.5 font-mono text-[0.62rem] tracking-wider text-content-subtitle uppercase transition-colors hover:border-neon/60 hover:text-neon',
                className,
            )}
        >
            {children}
