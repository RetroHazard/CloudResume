...
import { defaultRingColors, type RingContextValue, type RingData, RingProvider, ringCssVars } from './ring-context';
...
function isRingCenter(child: ReactNode): boolean {
    return (
        isValidElement(child) &&
        typeof child.type === 'function' &&
        ((child.type as { displayName?: string }).displayName === 'RingCenter' || child.type.name === 'RingCenter')
    );
}
            ...
            return {
                ...
                progressPath:
                    progressEndAngle <= startAngle + 0.01
                        ? ''
                        : generateRingArcPath(innerRadius, outerRadius, startAngle, progressEndAngle, cornerRadius),
