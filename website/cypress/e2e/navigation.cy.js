describe('Page navigation', () => {
    ...
    pages.forEach(({ label, path }) => {
        ...
        it(`marks ${label} link as active when on ${path}`, () => {
            ...
            cy.get(`nav[aria-label="Primary navigation"] a[href="${path}"]`).should('have.class', 'nav-block-active');
        });
    });

    it('marks Home link as active when on /', () => {
        cy.visit('/');
        cy.get('nav[aria-label="Primary navigation"] a[href="/"]').should('have.class', 'nav-block-active');
    });
